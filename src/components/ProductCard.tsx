  ...
  return (
    <Card className="flex flex-col overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 rounded-lg border-2 border-transparent hover:border-accent group">
      <CardHeader className="p-0 relative">
        <Link href={`/product/${product.id}`}>
            <Image 
              ...
              data-ai-hint={`${product.name.split(' ')[0].toLowerCase()} ${product.category.slice(0, -1).toLowerCase()}`} 
            />
        </Link>
      ...
      <CardFooter className="p-4 flex justify-between items-center">
        ...
        <Button onClick={() => addToCart(product)} className="bg-primary hover:bg-primary/90 text-primary-foreground">
