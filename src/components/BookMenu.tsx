...
const Page = React.forwardRef<HTMLDivElement, { children: React.ReactNode, className?: string }>(({ children, className }, ref) => {
  return (
    <div ref={ref} className={cn("book-page__page", className)}>
                                ...
                                <div className="book-page__back">
                                    <div className="book-page__content">
                                        <div className="p-2 sm:p-4 h-full overflow-y-auto custom-scrollbar">
                                            <h3 className="font-headline text-2xl font-bold p-4 sticky top-0 bg-background/80 backdrop-blur-sm z-10">{category.name}</h3>
                ...
                <div className="book__tabs">
                    {productsByCategory.map((cat, index) => (
                        <button 
                            ...
                            className={cn(
                                "book__tab",
                                currentPage === index && "book__tab--active"
                            )}
